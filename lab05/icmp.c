...
void ping(int sockfd, struct in_addr ip_addr, int count)
{
	...
	while (1) {
		...
		*/
		...
		printf("%lu bytes from %s: icmp_seq=%d ttl=%d time=%lf ms\n", reply.size, ip_s, i, rip_hdr->ttl ,time_elapsed);
...
void traceroute(int sockfd, struct in_addr ip_addr)
{
	...
	while (1) {
		...
		*/
		...
		current_addr.s_addr = rip_hdr->saddr;
		printf("%d %s %lf\n", my_ttl, inet_ntoa(current_addr), time_elapsed);
