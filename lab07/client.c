...
void run_client(int sockfd) {
    ...
    while (read(STDIN_FILENO, buf, BUFLEN - 1) > 0 && !isspace(buf[0])) {
        ...
        DIE(bytes_send < 0, "cannot send to server");

        // TODO 5: Receptionati un mesaj venit de la server

        memset(buf, 0, BUFLEN);
        bytes_received = recv(sockfd, buf, BUFLEN, 0);
        DIE(bytes_received < 0, "cannot received from server");
...
int main(int argc, char* argv[])
{
    ...
    DIE(sockfd < 0, "cannot open client socket");
    ...
    DIE(c < 0, "cannot connect to server");
